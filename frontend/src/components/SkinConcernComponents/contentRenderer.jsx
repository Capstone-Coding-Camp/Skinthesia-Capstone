...
import ImageBlock from './imageBlock'; // Tambahkan jika ingin gambar di dalam array content
import List from './list';
import Heading from './heading';

const ContentRenderer = ({ blocks }) => {
  if (!blocks || !Array.isArray(blocks)) {
    return null; // Atau pesan error
  }

  return (
    <>
      {blocks.map((block, index) => {
        const key = `${block.type}-${index}`; // Fallback key
