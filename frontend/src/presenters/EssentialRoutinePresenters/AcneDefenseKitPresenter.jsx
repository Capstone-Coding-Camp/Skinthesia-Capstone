...
import HomePageView from '@views/EssentialRoutine/EssentialRoutine'; // Import komponen View
import { essentialRoutineModel } from '@models/essentialRoutineModel'

const AcneDefenseKitPresenter = () => {
  ...
  useEffect(() => {
    try {
      ...
      setMorningRoutineSteps(rawData[2]);
      setLoading(false);
    } catch (err) {
      setError(err);
      setLoading(false);
...
export default AcneDefenseKitPresenter;